package org.journey.mingrui.web.business.basic.error.controller;
...
public class Error {
    ...
    public ModelAndView error404(RedirectAttributes attr, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (isAjax(request)) {
            returnJson(response, WebExceptionConstants.NOT_FOUND_CODE, WebExceptionConstants.NOT_FOUND_MSG);
            return null;
        } else {
    ...
    public ModelAndView error403(RedirectAttributes attr, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (isAjax(request)) {
            returnJson(response, WebExceptionConstants.CSRF_ERROR_CODE, WebExceptionConstants.CSRF_ERROR_MSG);
            return null;
        } else {
